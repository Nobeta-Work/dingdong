...
| 实训交付形式 | Docker Compose 一键启动、初始化数据、买家端与管理端页面 |
...
- 实训交付必须支持 Docker Compose 一键启动；公网部署推迟到简历版完成后。
...
| 07-18 至 07-22 | v0.6 前端并行 | OpenAPI 持续生成、买家端与管理端页面、接口联调 |
| 07-22 | v0.8 集成冻结 | 冻结新功能，修复阻塞问题，准备初始化数据和演示流程 |
| 07-23 | v0.9 实训交付 | Docker Compose 启动、前后端页面展示、核心交易闭环验收 |
...
| 版本 | 目标 | 主要交付物 |
|---|---|---|
| v1.0 | 简历完善版 | 完整补偿与故障测试、并发报告、架构说明、部署说明、演示视频、公网部署准备 |
| v1.1 | 秒杀增强版 | Redis Lua 资格校验、RocketMQ 异步下单、限购、防超卖与失败回补 |
...
> [!success] 关键范围已经确认
> 截止日期、学校要求、Docker Compose 交付、模拟支付、公网部署优先级和 P2 秒杀方向均已确认。当前不存在阻塞开发启动的产品决策。

尚未锁定但不阻塞开发的技术项：

- Dubbo 3.3.x 具体小版本，由 v0.1 最小 POC 结果决定；
