...
| 等级 | 定义 | 交付要求 |
|---|---|---|
| P0 | 核心交易闭环 | v1.0 必须全部完成 |
| P1 | 分布式项目核心亮点 | v1.0 必须全部完成 |
| P2 | 增强功能 | v1.0 后至多选择一项实现 |
| P3 | 远期需求 | 当前不开发，仅保留方向 |
...
原始实训题目覆盖会员、商品、促销、支付、物流、推荐和大数据分析等大量功能，完整实现远超四人实训团队的时间与能力范围。团队成员技术水平差异较大，因此项目需要同时满足：

1. 能按期完成并用于实训验收；
2. 核心业务可以独立演示，不依赖真实第三方平台；
3. 架构和关键实现具备简历与面试讨论价值；
4. 普通 CRUD 可拆分给基础较弱的成员；
5. 核心交易、分布式一致性和公共基础设施由项目负责人统一把控。
...
v1.0 完成后，根据时间只选择一个方向：

1. **秒杀活动**：Redis Lua 原子预扣、RocketMQ 异步创建订单、限购与防重复；
2. **优惠券**：发放、领取、锁定、核销、过期和取消回退；
3. **商品评价**：已完成订单评价、评分汇总、重复评价限制；
4. **行为事件**：浏览、加购、下单事件采集与简单热度排行。
...
- 公共前缀：`/api/v1`。
- 买家资源示例：`/api/v1/products`、`/api/v1/cart/items`、`/api/v1/orders`。
- 管理资源前缀：`/api/v1/admin`。
...
详细 Git 规则见 [[GIT|Git 流程说明]]，人员职责见 [[TEAM|团队协作说明]]。

### 13.1 推荐分工

| 角色 | 建议职责 |
|---|---|
| 项目负责人 | 架构、父工程、Gateway、认证、Nacos、Dubbo、RocketMQ、订单状态机、库存一致性、代码审核与集成 |
| 成员 A | 用户资料、收货地址、管理员用户查询 |
| 成员 B | 分类、品牌、SPU/SKU 普通 CRUD 与接口测试 |
| 成员 C | 购物车、订单列表/详情、管理端查询等模板化业务 |

### 13.2 协作规则

- 负责人先提供模块模板、代码规范和接口契约，成员在模板内开发。
- 核心状态、公共 DTO、数据库表和消息结构不得由成员单方面修改。
- 每个功能以小批次提交，并同时提交接口文档与最小测试。
- 合并前至少通过编译、单元测试和核心接口自测。
- 一项功能只能有一个明确负责人，避免多人同时修改同一业务文件。

## 14. 版本计划与验收

| 版本 | 目标 | 主要交付物 |
|---|---|---|
| v0.1 | 工程基线 | 父 POM、服务骨架、统一响应、异常、Nacos、Docker Compose、健康检查 |
| v0.2 | 用户与商品 | 登录鉴权、用户地址、分类品牌、SPU/SKU、管理接口 |
| v0.3 | 基础交易 | 商品浏览、购物车、订单创建与查询、库存锁定 |
| v0.4 | 支付履约 | 模拟支付、支付事件、发货、确认收货、状态日志 |
| v0.5 | 分布式可靠性 | 延时关单、事务消息、幂等、重试、补偿、缓存治理 |
| v0.6 | 前端与质量 | OpenAPI 冻结、买家端、管理端、集成测试、演示数据 |
| v1.0 | 可交付版本 | 全流程验收、部署说明、测试报告、架构图、简历说明 |
...
| 风险 | 影响 | 应对 |
|---|---|---|
| 团队成员基础薄弱 | 关键模块延期或质量不稳定 | 负责人先搭模板，成员承担边界清晰的 CRUD，强制评审 |
| Dubbo 与具体依赖小版本兼容性 | 服务间调用无法稳定运行 | v0.1 先完成最小 POC，再锁定 Dubbo 与 Nacos 小版本 |
...
> - 实训最终截止日期与每周可投入时间；
> - 是否需要部署到公网供面试演示；
> - P2 最终选择秒杀、优惠券、评价还是行为事件；
> - 学校验收是否强制要求某些原题功能或指定页面。
