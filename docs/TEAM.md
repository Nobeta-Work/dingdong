...
| 后端业务 | 已完成主路径 | 用户、商品、购物车、订单、库存锁定、模拟支付、发货/收货、延时关单、Outbox 重试及商品详情缓存均已有实现与针对性单元测试。 |
| 前端 | 已搭建，待联调 | 买家端页面、商品管理页面和请求封装已存在；管理员订单页因缺少查询接口暂不可操作。 |
| API 文档 | 部分完成 | v0.2–v0.5 Markdown 契约已存在；尚未接入或生成 OpenAPI。 |
| 交付编排 | 未完成 | 当前 Compose 仅启动中间件，尚未覆盖五个 Java 服务与前端。 |

## 7. 交付前剩余清单

1. [ ] 补齐管理员订单列表与详情接口，使管理端订单查询、详情和发货可形成闭环。
...
7. [ ] 将 Java 服务与前端纳入 Compose，完成一次从空环境启动的 v0.9 验收。
