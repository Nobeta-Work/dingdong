...
| 买家端前端 | 联调完成 | 登录、商品、购物车、地址、下单、模拟支付和订单履约均已接入。 |
| 管理端前端 | 联调完成 | 商品、订单、用户、经营概览和秒杀控制台均已接入真实接口。 |
...
| 图床服务 | 独立文件服务接入 MinIO、S3 或现有图床，补齐类型校验、权限和失效清理 |
| 短信能力 | 接入云短信适配层，实现验证码发送、Redis TTL、频率限制和防刷 |
...
2. [x] 文件策略冻结为图片 URL；当前阶段不实现本地文件上传，也不依赖 `/api/files`。
...
3. 演示支付幂等、订单超时关闭、库存释放与秒杀最终一致性；不扩展真实支付、推荐或上传功能。
4. 按 README 从停止状态重新启动一次，复核演示账号、图片 URL 和关键页面。
