...
@Mapper public interface PaymentMapper {
 ...
 @Insert("insert ignore into payment_order(payment_no,order_no,user_id,amount,channel,status) values(#{paymentNo},#{orderNo},#{userId},#{amount},#{channel},#{status})") @Options(useGeneratedKeys=true,keyProperty="id") int insert(PaymentOrder payment);
 ...
 @Update("update payment_order set status='PENDING',transaction_no=null,paid_at=null where payment_no=#{paymentNo} and status='FAILED'") int reopenFailed(String paymentNo);
}
