...
class OrderServiceTest {
 @Test void createsOrderFromRemotePriceAndAddressSnapshot(){
  ...
  assertEquals(new BigDecimal("25.00"),created.getTotalAmount());verify(mapper).insertItem(argThat(i->i.getSkuId().equals(9L)&&i.getQuantity()==2&&i.getTotalAmount().equals(new BigDecimal("25.00"))));verify(cart).removeAfterOrder(1L,List.of(5L));verify(outbox).publish(11L);
 }
}
