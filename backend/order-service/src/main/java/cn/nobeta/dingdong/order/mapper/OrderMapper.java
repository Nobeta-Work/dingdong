...
public interface OrderMapper {
 ...
 @Select("select id,order_no,user_id,receiver_name,receiver_phone,receiver_address,total_amount,status,created_at from mall_order where order_no=#{orderNo} and user_id=#{userId}") MallOrder findOwned(@Param("orderNo") String orderNo,@Param("userId") Long userId);
 @Select("select id,order_no,user_id,receiver_name,receiver_phone,receiver_address,total_amount,status,created_at from mall_order where user_id=#{userId} order by id desc limit #{size} offset #{offset}") List<MallOrder> findPage(@Param("userId") Long userId,@Param("size") int size,@Param("offset") int offset);
